...
export default function PortfolioSkills() {
  ...
  return (
    <section id="skills" className="py-20 bg-slate-900/50" ref={ref}>
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <p className="text-xl text-slate-300 font-mono">Technologies I work with</p>
        </motion.div>
          ...
          >
            {skillCategories.map((category, index) => (
              <motion.button
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-lg border transition-all duration-300 font-mono ${
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {skillCategories[activeCategory].skills.map((skill, index) => (
                <motion.div
                  ...
                  className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/30 hover:border-green-500/50 transition-all duration-300 group cursor-pointer"
                  whileHover={{ scale: 1.05, y: -5 }}
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-green-400 rounded-full group-hover:animate-pulse"></div>
                    <span className="text-white font-mono group-hover:text-green-400 transition-colors">{skill}</span>
                  </div>
                </motion.div>
              ))}
            </div>
